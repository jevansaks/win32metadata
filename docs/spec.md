...
methods with `[DllImport]` attributes. The primary DLL mapping comes from
`libMappings.rsp` (auto-generated by scanning `.lib` files). Manual overrides for
functions the auto-scan gets wrong or misses are applied via the
`_Win32_metadata_import_library_("dll.dll")` inline header annotation on the
owning declaration (see `docs/copilot/libmappingsmanual-migration.md`); the
former `libMappingsManual.rsp` sidecar has been fully migrated and removed.

Additional function metadata is applied via memberRemap entries:
- Parameter type overrides — from `emitter.settings.rsp`

`SetLastError` is applied via the `_Win32_metadata_set_last_error_` inline header
annotation on the owning SDK declaration (see
`docs/copilot/withsetlasterror-migration.md`); the former `WithSetLastError.rsp`
sidecar has been fully migrated and removed.
