...
| `manual.h` | Com.StructuredStorage | matched |  |  | Investigated; single abstract C++ interface (IMemoryAllocator) with pure-virtual methods only (blocker class 5), no ownership metadata gap. See docs/copilot/header-reports/manual.h.md |
...
| `stdint.h` | AllJoyn | matched |  |  | Investigated; standard C99 fixed-width integer typedefs only (no functions), classified by standard-library knowledge since only referencing partition (AllJoyn) cannot build. See docs/copilot/header-reports/stdint.h.md |
