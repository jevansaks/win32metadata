...
- **Partitions:** FileHistory, InternetConnectionWizard
- **Delta:** No retained patch previously existed. Header-scraping investigation (x86
  ...
  0 warnings/errors. This header declares the legacy Internet Connection Wizard COM
  interface API surface; static review confirms no `DECLARE_HANDLE` typedefs and no
  `_Out_`/`_Outptr_ HANDLE` output-parameter patterns.
...
- **Evidence:** Live scrape (`dotnet build generation/WinSDK -c Release -p:ScanArch=x86
  -t:ScrapeHeaders -p:PartitionFilter=InternetConnectionWizard`) succeeds with 0
