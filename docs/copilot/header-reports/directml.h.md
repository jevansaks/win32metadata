...
- **Delta:** No retained patch previously existed. Header-scraping investigation (x86
  ...
  0 warnings/errors. This header declares the DirectML COM interface API surface
  (`DMLCreateDevice`, `IDMLDevice`, etc.); static review confirms no `DECLARE_HANDLE`
...
- **Evidence:** Live scrape (`dotnet build generation/WinSDK -c Release -p:ScanArch=x86
  -t:ScrapeHeaders -p:PartitionFilter=DirectML`) succeeds with 0 warnings/errors. Static
