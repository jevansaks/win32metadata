...
- **Delta:** No retained patch previously existed. Header-scraping investigation (x86
  ...
  0 warnings/errors. This header declares the DirectX Shader Compiler (DXC) COM interface
  API surface (`DxcCreateInstance`, `IDxcCompiler`, etc.); static review confirms no
...
- **Evidence:** Live scrape (`dotnet build generation/WinSDK -c Release -p:ScanArch=x86
  -t:ScrapeHeaders -p:PartitionFilter=Direct3DDxc`) succeeds with 0 warnings/errors. Static
