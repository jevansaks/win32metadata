...
- **Delta:** No retained patch previously existed. Header-scraping investigation (x86
  scrape, the viable architecture for this `ExcludeFromCrossarch` partition, shared with
  `windowssideshowapi.h`/`windowssideshow.h` already classified in
  `scraping-investigation-06`) succeeded with 0 warnings/errors. This header declares the
  Windows SideShow driver-event notification COM interface API surface; static review
