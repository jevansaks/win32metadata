...
- **Delta:** No retained patch previously existed. Header-scraping investigation (x86
  ...
  0 warnings/errors. This header declares the `netsh.exe` helper-DLL entry-point API
  surface (8 functions); static review confirms no `DECLARE_HANDLE` typedefs and no
...
- **Evidence:** Live scrape (`dotnet build generation/WinSDK -c Release -p:ScanArch=x86
  -t:ScrapeHeaders -p:PartitionFilter=NetShell`) succeeds with 0 warnings/errors. Static
