...
- **Delta:** No retained patch previously existed. Header-scraping investigation (x86
  ...
  0 warnings/errors. The generated `RemoteAssist.cs` contains no `DllImport`/function
  declarations — this header declares only the Remote Assistance rendezvous-session COM
...
- **Evidence:** Live scrape (`dotnet build generation/WinSDK -c Release -p:ScanArch=x86
  -t:ScrapeHeaders -p:PartitionFilter=RemoteAssist`) succeeds with 0 warnings/errors;
  generated `obj/generated/common/RemoteAssist.cs` contains no `DllImport`/`public static
  extern` entries.
