...
- **Delta:** No retained patch previously existed. Header-scraping investigation (x86
  ...
  0 warnings/errors. This header declares the Mobile Broadband Network (MBN) COM interface
  API surface (`IMbnInterfaceManager`, etc.); static review confirms no `DECLARE_HANDLE`
...
- **Evidence:** Live scrape (`dotnet build generation/WinSDK -c Release -p:ScanArch=x86
  -t:ScrapeHeaders -p:PartitionFilter=Mbn`) succeeds with 0 warnings/errors. Static review
