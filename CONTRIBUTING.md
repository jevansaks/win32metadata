...
Run a [full build](#full-builds), fix any metadata issues that surface, address failures from tests, then update the baseline.
...
* If test reports missing entries in emptyDelegatesAllowList.rsp, the fix depends on the pattern: a bare function typedef with a separate pointer-adding alias typedef (`typedef NAME *ALIAS;`) is auto-discovered from AST structure with no configuration needed. Two patterns can't be determined from the AST alone and need an explicit annotation directly on the producer declaration instead: a same-level alias (`typedef NAME ALIAS;`, no added pointer) needs `_Win32_metadata_canonical_name_(ALIAS)` on the non-canonical typedef; a struct field using the bare typedef directly with no alias anywhere (`NAME *fieldName;`) needs `_Win32_metadata_reduce_pointer_level_` on that field. See [docs/generationOptions.md](docs/generationOptions.md#function-pointer-fixups) for examples; the former `functionPointerFixups.json` sidecar has been fully migrated and removed.
* Additional entries for entrypoints not present in import libs should be applied via a `_Win32_metadata_import_library_("dll.dll")` annotation directly on the owning declaration in the header (see [docs/copilot/libmappingsmanual-migration.md](docs/copilot/libmappingsmanual-migration.md) for the convention and examples; the former `libMappingsManual.rsp` sidecar has been fully migrated and removed). You probably won't know you need to add this until someone reports a bug that an API isn't present because it's a LoadLibrary + GetProcAddress only API. If the API has no header declaration at all (a true LoadLibrary + GetProcAddress-only export with no public prototype), add it as hand-written C# under `generation/WinSDK/manual/` with a `[DllImport("dll.dll", ...)]` attribute instead - do not fabricate a header declaration without a verified authoritative signature.
